...
public static String toHtml(Spanned text) {
    ...
    private static void withinHeader(StringBuilder out, Spanned text,
                                     int start, int end){
        int next;
        for(int i = start; i < end; i = next){
            next = text.nextSpanTransition(i, end, RelativeSizeSpan.class);
            RelativeSizeSpan[] sizeSpans = text.getSpans(i, next, RelativeSizeSpan.class);
        ...
        for (int i = start; i < end; i = next) {
            ...
            for (int j = 0; j < style.length; j++) {
                ...
                if (style[j] instanceof RelativeSizeSpan){
                    float sizeChange = ((RelativeSizeSpan) style[j]).getSizeChange();
                    if(sizeChange == CustomStyles.TITLE){
                        out.append("<h1>");
                    }
                    if(sizeChange == CustomStyles.SUBTITLE){
                        out.append("<h2>");
                    }
                }
